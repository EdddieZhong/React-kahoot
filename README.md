...
To run the backend server, simply run `yarn backend` in the `backend` directory. This will start the backend, to set up the backend you must run `yarn install` in `backend` directory once..
